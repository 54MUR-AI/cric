  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-stone-800 dark:text-stone-200">Maintenance</h1>
            ...
            <div className="rounded-lg bg-white dark:bg-stone-900 p-4 shadow-sm dark:shadow-black/20 border border-stone-200 dark:border-stone-700 flex items-center justify-between cursor-pointer hover:border-emerald-300 dark:hover:border-emerald-600 transition-colors" onClick={() => openTask(task)}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-stone-800 dark:text-stone-200 truncate">{task.title}</span>
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusColors[task.status]}`}>{task.status.replace('_', ' ')}</span>
                </div>
                <div className="flex gap-3 mt-1 text-xs text-stone-400 dark:text-stone-500">
              ...
              <MessageCircle className="h-4 w-4 text-stone-400 dark:text-stone-500 shrink-0 ml-2" />
            </div>
          </SwipeToDelete>
        ))}
          ...
          <div className="w-full max-w-lg rounded-lg bg-white dark:bg-stone-900 p-6 shadow-xl dark:shadow-black/30" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full rounded-md border border-stone-300 dark:border-stone-600 px-3 py-2 text-sm" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Description</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full rounded-md border border-stone-300 dark:border-stone-600 px-3 py-2 text-sm" rows={3} />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Due date</label>
                <input type="date" value={formData.due_date} onChange={(e) => setFormData({ ...formData, due_date: e.target.value })} className="w-full rounded-md border border-stone-300 dark:border-stone-600 px-3 py-2 text-sm" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setSelectedTask(null)}>
          <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-lg bg-white dark:bg-stone-900 p-6 shadow-xl dark:shadow-black/30" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t border-stone-200 dark:border-stone-700 pt-4">
              <h3 className="font-medium text-stone-700 dark:text-stone-300 mb-3">Comments ({comments.length})</h3>
              <div className="space-y-3 mb-3 max-h-48 overflow-y-auto">
                {comments.map((c) => (
                  <div key={c.id} className="text-sm">
                    <span className="font-medium text-stone-700 dark:text-stone-300">{c.profiles?.display_name || 'Unknown'}</span>
                    <span className="text-stone-400 dark:text-stone-500 ml-2 text-xs">{formatDate(c.created_at)}</span>
                    <p className="text-stone-600 dark:text-stone-400 mt-0.5">{c.body}</p>
                  </div>
                ))}
                {comments.length === 0 && <p className="text-sm text-stone-400 dark:text-stone-500">No comments yet</p>}
              </div>
              <div className="flex gap-2">
                <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Add a comment..." className="flex-1 rounded-md border border-stone-300 dark:border-stone-600 px-3 py-2 text-sm" onKeyDown={(e) => e.key === 'Enter' && addComment()} />
