    ...
    async function check() {
      try {
        const [bookings, tasks, trips] = await Promise.all([
          supabase.from('bookings').select('id, cabin_id, user_id, start_date, end_date').gt('created_at', cutoff).order('created_at', { ascending: false }),
        ...
        let cabinMap = new Map()
        const cabinIds = [...new Set((bookings.data || []).map(b => b.cabin_id).filter(Boolean))]
        if (cabinIds.length) {
          const { data: cabins } = await supabase.from('cabins').select('id, name').in('id', cabinIds)
          cabinMap = new Map((cabins || []).map(c => [c.id, c.name]))
        }
        ...
        newBookings.forEach(b => {
          ids.push(`b-${b.id}`)
          toast.info(`${cabinMap.get(b.cabin_id) || 'Cabin'} booked ${b.start_date?.slice(5)}–${b.end_date?.slice(5)}`)
        })
        newTasks.forEach(t => {
          ids.push(`t-${t.id}`)
