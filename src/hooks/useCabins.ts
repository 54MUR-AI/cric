...
export function useCabins() {
  ...
  async function updateCabin(id: string, updates: Partial<Cabin>) {
    ...
    const { data, queued } = await offlineUpdate('cabins', id, updates)
    if (queued) {
      db.cabins.put({ ...original!, ...updates })
      toast.info('Cabin update queued — will sync when online')
    } else if (data) {
  ...
  async function deleteCabin(id: string) {
    ...
    try {
      const { queued } = await offlineDelete('cabins', id)
      db.cabins.delete(id)
      toast.info(queued ? 'Cabin deletion queued — will sync when online' : 'Cabin deleted')
  ...
  async function addImprovement(cabinId: string, year: number, description: string) {
    const { data, queued } = await offlineInsert('cabin_improvements', { cabin_id: cabinId, year, description })
    if (data) {
      ...
      setCabins((current) => current.map((c) => c.id === cabinId
        ...
        : c))
      toast.info(queued ? 'Maintenance queued — will sync when online' : 'Maintenance added')
    }
    return data
  }

  async function deleteImprovement(id: string, cabinId: string) {
    const { queued } = await offlineDelete('cabin_improvements', id)
    if (!queued) {
      ...
      setCabins((current) => current.map((c) => c.id === cabinId
        ? { ...c, improvements: (c.improvements || []).filter((i) => i.id !== id) }
        : c))
      toast.info('Maintenance delete queued — will sync when online')
    }
  }
